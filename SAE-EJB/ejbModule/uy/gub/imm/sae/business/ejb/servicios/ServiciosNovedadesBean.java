...
public class ServiciosNovedadesBean {
	...
	private static Logger logger = Logger
			.getLogger(ServiciosNovedadesBean.class);

	private static final DateFormat FULL_TIME_DF = new SimpleDateFormat(
			"yyyyMMdd HHmm ZZZ");
	 ...
	 * @param paso
	 */
	public void publicarNovedad(Empresa empresa, Reserva reserva,
			Acciones accion) {

		logger.debug("Novedad recibida para publicar: reserva "
				+ reserva.getId());
		...
		try {
			novedad.setTimestamp(DatatypeFactory.newInstance()
		...
		novedad.setNumeroReserva(reserva.getNumero() == null ? "" : reserva
				.getNumero().toString());
		...
		for (DatoReserva valor : reserva.getDatosReserva()) {
			if (valor != null && valor.getId() != null
					&& valor.getDatoASolicitar() != null) {
				if (valor.getDatoASolicitar().getIncluirEnNovedades() != null
						&& valor.getDatoASolicitar().getIncluirEnNovedades()
								.equals(Boolean.TRUE)) {
					Publicar.Datosextras.Datoextra dato = objFactory
							.createPublicarDatosextrasDatoextra();
					dato.setNombre(valor.getDatoASolicitar().getNombre());
					dato.setTipo(valor.getDatoASolicitar().getTipo()
							.getDescripcion());
		...
		Integer novedadId = registrarNovedad(empresa, reserva,
				novedadToXml(novedad));

		if (novedadId != null) {
			logger.debug("La novedad fue registrada en la base de datos (id="
					+ novedadId + ", no enviada aún).");
		} else {
	...
	private String novedadToXml(Publicar novedad) {
		...
		try {
			...
			logger.warn(
					"No se pudo convertir la novedad a XML: " + ex.getMessage(),
					ex);
		}
		return null;
	}
	...
	private Integer registrarNovedad(Empresa empresa, Reserva reserva,
			String datos) {
		if (datos == null) {
			return null;
		}
		...
		try {
			novedad_xml_log = confBean.getBoolean("WS_NOVEDADES_XML_LOG");
		} catch (Exception nfEx) {
	...
	private Publicar xmlToNovedad(String xml) {
		...
		try {
			...
			logger.warn(
					"No se pudo convertir el XML a la novedad: "
							+ ex.getMessage(), ex);
		}
		return null;
	}
	...
	public void enviarNovedadesPendientes() {
		...
		boolean lockOk = (boolean) globalEntityManager.createNativeQuery(
				"SELECT pg_advisory_unlock(" + LOCK_ID + ")").getSingleResult();
		// Intentar obtener el lock
		lockOk = (boolean) globalEntityManager.createNativeQuery(
		...
		try {
			...
			int maxIntentos = 15;
			try {
				maxIntentos = confBean.getLong("WS_NOVEDADES_MAXINTENTOS")
						.intValue();
			} catch (Exception nfEx) {
				maxIntentos = 15;
			}
			...
			logger.debug("Consulta para determinar las novedades pendientes: "
					+ eql);
			
			int maxResults = 500;
			try {
				maxResults = confBean.getLong("WS_NOVEDADES_MAXRESULTS").intValue();
			} catch (Exception nfEx) {
				maxResults = 500;
			}
			...
			logger.debug("Se encontraron " + novedades.size()
					+ " novedades pendientes de envío.");

			if (!novedades.isEmpty()) {
				URL urlWsdl = NuevaNovedadService_Service.class
						.getResource("PublicacionTopico-SAENovedades_v2.wsdl");
				NuevaNovedadService_Service novedadService = new NuevaNovedadService_Service(
						urlWsdl);
				NuevaNovedadService novedadPort = novedadService
						.getNuevaNovedadPort();
				try {
			...
			globalEntityManager.createNativeQuery(
	...
	/*
	 * Atención que no es exactamente igual al método del mismo nombre de otros
	 * servicios (ServiciosTrazabilidadBean) porque este incluye un handler
	 * exclusivo para novedades!!
	 */
	...
	private DateTime configurarWSPort(Object port) throws Exception {
		...
		SAMLAssertion tokenSTS = obtenerTokenSTS(urlSts, rol, wsaTo, policy,
				orgKsPath, orgKsPass, orgKsAlias, sslKsPath, sslKsPass,
				sslKsAlias, sslTsPath, sslTsPass, timeout);

		AddressingBuilder addrBuilder = SOAPAddressingBuilder
				.getAddressingBuilder();
		SOAPAddressingProperties addrProps = (SOAPAddressingProperties) addrBuilder
		...
		customHandlerChain.add(new PublishSubscribeHeadersHandler(productor,
				topico)); // Para P&S
		...
		tlsParams.setSSLSocketFactory(createSSLContext(sslKsPath, sslKsPass,
				sslKsAlias, sslTsPath, sslTsPass).getSocketFactory());
		conduit.setTlsClientParameters(tlsParams);
		bindingProvider.getBinding().setHandlerChain(customHandlerChain);

		// Se le quita algunos segundos para prevenir que venza mientras se
		// ejecuta alguna acción
		DateTime tokenVence = tokenSTS.getAssertion().getConditions()
				.getNotOnOrAfter().minusSeconds(3);

		return tokenVence;
	}

	private SAMLAssertion obtenerTokenSTS(String urlSts, String rol,
			String wsaTo, String policy, String orgKsPath, String orgKsPass,
			String orgKsAlias, String sslKsPath, String sslKsPass,
			String sslKsAlias, String sslTsPath, String sslTsPass, int timeout)
			throws Exception {
		...
		try {
			assertion = client.requestSecurityToken(bean, orgKeyStore,
					sslKeyStore, sslTrustStore, urlSts);
		} catch (Exception ex) {
	...
	private SSLContext createSSLContext(String sslKsPath, String sslKsPass,
			String sslKsAlias, String sslTsPath, String sslTsPass)
			throws Exception {
		KeyStore trustStore = KeyStore.getInstance(KeyStore.getDefaultType());
		trustStore
				.load(new FileInputStream(sslTsPath), sslTsPass.toCharArray());
