  ...
  private EntityManager globalEntityManager;
	...
	public void eliminarReservasPendientes(){
		...
		//Consulta para eliminar las disponibilidades ocupadas por reservas individuales
      	String sql1 = "DELETE FROM {esquema}.ae_reservas_disponibilidades rd WHERE rd.aedi_id IN ("
      	...
      	String sql2 = "DELETE FROM {esquema}.ae_datos_reserva dr WHERE dr.aers_id IN ( :idsReservas )";
			
		//Consulta para eliminar las reservas individuales
		String sql3 = "DELETE FROM {esquema}.ae_reservas r WHERE r.id IN ( :idsReservas )";
			
      	//Consulta para eliminar las disponibilidades ocupadas por reservas múltiples
      	String sql4 = "DELETE FROM {esquema}.ae_reservas_disponibilidades rd WHERE rd.aers_id IN ("+idsRecursosMultiples+")";
      
		//Consulta para eliminar los datos asociados a las reservas múltiples 
      	String sql5 = "DELETE FROM {esquema}.ae_datos_reserva d WHERE d.aers_id IN ("+idsRecursosMultiples+")";
			
      	//Consultas para eliminar las reservas múltiples pertenecientes a tokens vencidos
      	String sql6 = "DELETE FROM {esquema}.ae_reservas r WHERE r.id IN ("+idsRecursosMultiples+")";
			
      	//Consulta para eliminar los tokens de reserva múltiple
      	String sql7 = "DELETE FROM {esquema}.ae_tokens_reservas t WHERE t.aere_id=:idRecurso AND  t.estado = 'P' AND "
      ...
      for(Empresa empresa : empresas) {
        if(empresa.getDatasource() != null) {
    		...
    		Query queryRecursos =  globalEntityManager.createNativeQuery("SELECT id,reserva_pen_tiempo_max,reserva_multiple_pend_tiempo_max FROM  "+empresa.getDatasource()+".ae_recursos WHERE fecha_baja IS NULL ");
        	...
        	if(recursos!=null && !recursos.isEmpty()){
		      		for(Object[] recurso : recursos) {
		      			...
		      			logger.info("EMPRESA ID: " + empresa.getId() +"..."+ "RECURSO ID: " + idRecurso);
						  ...
						  if(tiempoMaxIndividual!=null && tiempoMaxIndividual>0){
							  logger.info("RECURSO ID: " + idRecurso + " TIEMPO MAX INDIVIDUAL "+ tiempoMaxIndividual+" .... ");	
							  tiempoMaximoIndividual = tiempoMaxIndividual;
						  	
						  }
						  
						  if(tiempoMaxMultiple!=null && tiempoMaxMultiple>0){
							  logger.info("RECURSO ID: " + idRecurso + " TIEMPO MAX MULTIPLE "+ tiempoMaxMultiple+" .... ");  
				          ...
				          try {
				            ...
				            //Consulta de ids de reservas individuales a eliminar
				            ...
				            ids = (List<Integer>) query1.getResultList();
				            System.out.println("ids" + ids.toString());
				            
				            sql = sql1.replace("{esquema}", empresa.getDatasource());
				            ...
				            if(!ids.isEmpty()){
					            ...
					            query1 = entityManager.createNativeQuery(sql);
					            query1.setParameter("idsReservas",ids);
					            query1.executeUpdate();
