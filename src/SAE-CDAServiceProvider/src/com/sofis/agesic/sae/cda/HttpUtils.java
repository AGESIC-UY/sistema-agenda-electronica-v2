...
public class HttpUtils {
	 ...
	 * so any + characters are converted to spaces, and characters sent in
	 * hexadecimal notation (like <i>%xx</i>) are converted to ASCII
	 * characters.
	 *
	 ...
	 * array of strings containing the multiple values sent by the POST
	 * method.
	 *
	 * <p>
	 * The keys and values in the hashtable are stored in their decoded form,
	 ...
	 *
	 *
	 * @exception IllegalArgumentException	if the data sent by the POST method
	 * is invalid
	 *
	 */
	static public Map<String, String[]> parsePostData(int len,
		  ServletInputStream in) {
		// XXX
		// should a length of 0 be an IllegalArgumentException

		if (len <= 0) {
			return new HashMap<>(); // cheap hack to return an empty hash
		}
		if (in == null) {
			throw new IllegalArgumentException();
		}
...
//			String postedBody = new String(postedBytes, 0, len, "utf-8");
